...
plugin = ($) ->
	...
	class Select7
		...
		constructor: (@el, config) ->
			...
			@config.removeCurrent = not @$el.is ".select7_no-remove_current"
		...
		open: ->
			...
			generate$optgroup = (optgroup) =>
				...
				$optgroup.attr("data-group", optgroup.group) if optgroup.group
				$optgroup.attr("data-parent", optgroup.parent) if optgroup.parent
				hasCurrent = no
			...
			@$drop.find('.select7__optgroup').each ->
				parent = $(@).data 'parent'
				$(@).parent().find('.select7__optgroup[data-group=' + parent + ']').find('>.select7__optgroup-items').append($(@)) if parent
				$(@).parents('.select7__optgroup_collapse').addClass('select7__optgroup_collapse_open') if $(@).hasClass 'select7__optgroup_collapse_open'
			@$drop.on "click", ".select7__option", (e) =>
				...
				@close()
			@$drop.on "click", ".select7__optgroup-label", (e) =>
	...
	$.fn.select7 = (method, args...) ->
		@each ->
			if $(@).find('option').length == 1
				$(@).hide()
				$(@).parent().append('<div class="select7 disabled"><div class="select7__current"><span data-role="value" class="select7__current-value" data-value="0">' + $(@).parent().find('option').text() +  '</span></div></div>')
			else
